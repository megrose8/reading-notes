# My Code Fellows Reading Notes

## About Me 
My name is Meg and I live in Portand, Oregon, but I grew up in Eastern Washington. I'm a recent college graduate and work for a non-profit teaching various computer classes to members of the community. 
I've dipped my toes into various fields including working as a project manager for a marketing consultancy, co-founding a interdisplinary humanities lab, and now- teaching! I've ended up in a coding bootcamp with the intention of eventually teaching young adults coding to provide them with marketable skills as the enter the workplace. Outside of work, I am a dog mom, nap enthusiast, and lover of brunch.

![small square](https://github.com/megrose8/reading-notes/assets/142434435/6fe9895a-71d9-49e1-9cf2-70ff5fa24ae5)
![square 1](https://github.com/megrose8/reading-notes/assets/142434435/980f5ec0-1fd0-4afd-9f2a-50a0965cd894)
![thumbnail_IMG_7569 small square](https://github.com/megrose8/reading-notes/assets/142434435/024ea006-57bd-4d1e-9337-6f77c868596e)



## Growth Mindset

*Thinking About the Growth Mindset*

For me, having a growth mindset means to have confidence in oneself alongside the ability to recognize, without judgement, one's strengths, and a willingness to engage with potential spaces for improvement.
A growth mindset resists the temptation to give up when things are hard, and instead encourages the radical act of self love, and the continual need for growth without judgement.

<img align="center">![an artists rendering on the concept of growth mindset with a face smiling and plants growing out of the head.](https://s39613.pcdn.co/wp-content/uploads/2022/06/creativity-personal-development-individual-growth-concept-vector-id1341427506.jpg)

*For When Things Get Hard Remember:*
- No rain, no flowers
- Failure is An opportunity for growth
- I am inspired by the success of others


## My Links:
[LinkedIn](https://www.linkedin.com/in/meg-b-7b85101b1/)

[Github Profile](https://github.com/megrose8)
### Publications
[Confronting Rhetorical Violence in Response to the Catholic Sex Abuse Crisis](http://moses.creighton.edu/JRS/)

[Liturgy in the Shadow of Trauma](https://www.mdpi.com/2077-1444/13/7/583)

## Class Notes: 

## Markdown

+ **What is It?**
  + Markdown is a coding language that utilizes a simpler syntax to emphasize ease of readibility within code.
 + **Why Use It?**
   + Markdown is highly accessible since it is platform independent (meaning it can run on any device) and is *future proof* as it can still be read even if the application being previously used no longer functions.
  
## Markdown Syntax
#### Description, Syntax, Example

Biggest Heading, # , See Below Figure 1

Smallest Heading, ######, See Below Figure 2

Bold, **, **Example**

Italicize,  * *, *Example*

Both, *** ***, ***Example*** 

Adding a Link, [Language to Appear](Address of Link), Example: [My Github](https://github.com/megrose8).

For an Unordered List, Use +/-/or *, See Below Figure 3

#### Example Figures:

# Figure 1: Biggest Heading
###### Figure 2: Smallest Heading
+ Figure
+ Number
+ Three
+ Lists




